"""Pine, Arizona specialist agent.

This agent provides Pine-specific information and enhances existing agent outputs
...
# Pine-specific knowledge base
PINE_KNOWLEDGE = {
    "location": {
        "name": "Pine, Arizona",
        "coordinates": {"lat": 34.3847, "lon": -111.4551},
        "elevation": 5400,  # feet
        "region": "Gila County, Arizona",
        "country": "US",
        "proximity": {
            "payson": {"distance_miles": 15, "direction": "northwest"},
            "strawberry": {"distance_miles": 5, "direction": "southeast"},
            "phoenix": {"distance_miles": 105, "direction": "south"},
        },
    },
    "history": {
        "founded": "Late 1800s",
        "current_population": "~2,000 residents",
        "known_for": [
    ...
    "outdoor_activities": {
        ...
        "climbing": {
            "description": "Limited climbing opportunities in Pine area",
            "note": "Pine is not a major climbing destination. For extensive climbing, consider nearby areas or use tools to find current climbing opportunities.",
        },
        "cycling": {
        ...
        "paddling": {
            "description": "Limited paddling opportunities in Pine area",
            "note": "Pine is not a major paddling destination. For extensive paddling, consider nearby lakes or use tools to find current opportunities.",
        },
    },
    "attractions": {
        "natural": [
...
class PineAgent(LocationAgentBase):
    """Agent specialized in Pine, Arizona information and context."""
    ...
    def _get_default_knowledge(self) -> Dict[str, Any]:
        """Get Pine-specific knowledge base (fallback if external file not found)."""
        return PINE_KNOWLEDGE

    def _get_system_prompt(self) -> str:
        """Get the system prompt for the Pine agent."""
        return """You are a comprehensive guide for Pine, Arizona - a small mountain town
on the Mogollon Rim at 5,400 feet elevation.

Your role is to:
1. Use tools to gather real-time data about Pine (trails, restaurants, accommodations, etc.)
2. Select which tools are most relevant based on the user's query and activity type
3. Enhance tool results with Pine-specific knowledge from the knowledge base
4. Combine information from existing agents with Pine expertise
...
- For hiking: search_trails(activity_type="hiking", location="Pine, Arizona")
- For mountain biking: search_trails(activity_type="mountain_biking", location="Pine, Arizona")
- For climbing: search_trails(activity_type="climbing", location="Pine, Arizona") if available
- For cycling: search_trails(activity_type="cycling", location="Pine, Arizona")
- For paddling: search_trails(activity_type="paddling", location="Pine, Arizona") if available
- For trail running: search_trails(activity_type="trail_running", location="Pine, Arizona")
...
- ALWAYS use search_trails with activity_type="mountain_biking" and location="Pine, Arizona"
- Pine has trails including Highline Trail (50+ miles, intermediate/advanced, can be done in sections) and Mogollon Rim Trail
- Enhance tool results with knowledge base information about trail difficulty, length, elevation, and features

For Hiking Queries:
- Use search_trails with activity_type="hiking" and location="Pine, Arizona"
...
For Dining Queries:
- Use find_restaurants with location="Pine, Arizona"
- Provide context about small mountain town character

For Accommodation Queries:
- Use search_accommodations with location="Pine, Arizona"
...
- Key spots: Mogollon Rim, Tonto National Forest
...
- Access via Highway 87/260 from Phoenix (105 miles)
- Elevation 5,400 feet (cooler than Phoenix)
- Tonto National Forest - most trails require no permits for day use
- Close to Strawberry (5 miles) and Payson (15 miles)
...
- Highlight unique characteristics (Mogollon Rim, small mountain town, cooler than Phoenix)
- Provide practical tips (parking, permits, best times, access)

Provide comprehensive, accurate, practical information that combines real tool data with Pine-specific knowledge and context."""
